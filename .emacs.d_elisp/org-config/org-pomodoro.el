...
(defcustom org-pomodoro-ticking-sound (when load-file-name
  ...
  "The path to a sound file thatÂ´s to be played while a pomodoro is running."
  :group 'org-pomodoro
  :type 'file)

(defcustom org-pomodoro-sound (when load-file-name
                                (concat (file-name-directory load-file-name) "resources/bell.wav"))
  "The path to a sound file thatÂ´s to be played when a pomodoro was finished."
  :group 'org-pomodoro
  :type 'file)

(defcustom org-pomodoro-killed-sound nil
  "The path to a sound file, thatÂ´s to be played when a pomodoro is killed."
  :group 'org-pomodoro
  :type 'file)
...
(defcustom org-pomodoro-short-break-sound (when load-file-name
                                            (concat (file-name-directory load-file-name)
                                                    "resources/bell.wav"))
  "The path to a sound file thatÂ´s to be played when a break was finished."
  :group 'org-pomodoro
  :type 'file)
...
(defcustom org-pomodoro-long-break-sound (when load-file-name
  ...
  "The path to a sound file thatÂ´s to be played when a long break is finished."
  :group 'org-pomodoro
  :type 'file)
...
(defun handle-progress-notification ()
  (if (and
       (>= org-pomodoro-countdown 300)
       (>= (- last-progress-notification org-pomodoro-countdown) 300))
    (progn
      (setq last-progress-notification org-pomodoro-countdown)
      (setq info-str (if (eq :pomodoro org-pomodoro-state) "Pomodoro:" "Break:"))
      (org-pomodoro-notify info-str
			   (format " %02d:%02d"
			   (/ org-pomodoro-countdown 60) (% org-pomodoro-countdown 60)))
      )
    )
  )
...
This may send a notification, play a sound and start a pomodoro break."
  ...
  (if (zerop (mod org-pomodoro-count org-pomodoro-long-break-frequency))
    ...
    (if (not (equal org-pomodoro-short-break-length 0)) 
