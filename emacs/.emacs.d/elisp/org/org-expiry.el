...
(defcustom org-expiry-confirm-flag t 
  ...
  :type '(choice (const :tag "Always require confirmation" t) 
		 (const :tag "Do not require confirmation" nil) 
		 (const :tag "Require confirmation in interactive expiry process" interactive)) 
  :group 'org-expiry)

(defcustom org-expiry-advised-functions '(org-scheduled org-deadline org-time-stamp) 
...
restart `org-mode' if necessary." 
  ...
  (when arg 
    (add-hook 'org-mode-hook 
	      (lambda() 
		(add-hook 'before-save-hook 'org-expiry-process-entries t t)))
...
If there is no expiry date, use `org-expiry-expiry-date'." 
  (let* ((ex-prop org-expiry-expiry-property-name) 
	 ...
	 (cr (org-read-date nil t (or (org-entry-get (point) cr-prop t) 
				      "+0d"))) 
	 (ex-field (or (org-entry-get (point) ex-prop t) 
		       org-expiry-wait)) 
	 (ex 
	  (if (string-match "^[ \t]?[+-]" ex-field) 
...
`org-expiry-handler-functions'." 
  ...
  (save-excursion 
	...
	(if (equal expired 0) 
	  ...
	  (message "Processed %d on %d expired entries" processed expired))))))
...
update the date." 
  (interactive "P") 
  (let* ((d (org-entry-get (point) org-expiry-created-property-name)) d-time d-hour timestr) 
	...
	(progn
	  ...
	  (setq timestr
		...
		(if (equal arg '(16)) 
		    (concat "<" (org-read-date nil nil nil nil d-time d-hour) ">") 
		  (format-time-string (cdr org-time-stamp-formats))))
	  ;; maybe transform to inactive timestamp
	  (if org-expiry-inactive-timestamps 
	      (setq timestr (concat "[" (substring timestr 1 -1) "]"))) 
	  (save-excursion 
	    (org-entry-put (point) org-expiry-created-property-name timestr))
	  1)
      0)))
...
and insert today's date." 
  (interactive "P") 
  (let* ((d (org-entry-get (point) org-expiry-expiry-property-name)) d-time d-hour) 
    (setq d-time 
	  (if d 
	      (org-time-string-to-time d) 
	    (current-time))) 
	...
	(setq timestr (concat "[" (substring timestr 1 -1) "]"))) 
    (save-excursion 
      (org-entry-put (point) org-expiry-expiry-property-name timestr))))
  ...
  (if (or (member keyword org-todo-keywords-1) 
	...
	(if (org-expiry-expired-p) 
	  ...
	  (if (org-called-interactively-p) 
	      (message "Entry at point is not expired.")))) 
    (error 
     "\"%s\" is not a to-do keyword in this buffer"
     keyword)))
